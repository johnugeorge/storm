            ...
            if op['function_type'] == 'Create':
                if op['function_arg'] == 'Table':
                    if op['function_subtype'] == 'Input':
                        ...
                        #input_data = DataFrame.from_dict(message['input'])
                        #d = {'one' : [1., 2., 3., 4.], 'two' : [4., 3., 2., 1.]}
                        #input_data = input_data.reindex_axis(message['cols'], axis=1)
                        input_data = DataFrame(message['input'], columns=message['cols'])
